...
1.  At the **Cloud Shell** command prompt in the portal, perform the following actions:
    ...
    d.  Enter the following command, and then select Enter to list just the namespaces of the currently registered providers:
...
In this exercise, you registered the resource provider for Content Delivery Network, and then used the provider to create both a CDN profile and endpoint resources.
