...
Given the dynamic nature of Microsoft cloud tools, you might experience Azure UI changes that occurred after this training content's development. These changes might cause the lab instructions and steps to not align correctly.
...
1. On the **Create a resource** pane, in the **Search services and marketplace** text box, enter **Event Grid Topic**, and then select Enter.

1. On the **Marketplace** search results pane, select the **Event Grid Topic** result, and then select **Create**.

1. On the **Create Topic** pane, on the **Basics** tab, perform the following actions, and then select **Next: Advanced**:
...
1. On the **Create a resource** pane, in the **Search services and marketplace** text box, enter **Web App**, and then select Enter.

1. On the **Marketplace** search results pane, select the **Web App** result, and then select **Create**.

1. On the **Create Web App** pane, on the **Basics** tab, perform the following actions, and then select **Next: Docker**:
   ...
   The following screenshot displays the configured settings on the **Create Web App** pane.

   ![Create Web App](./Media/l09_create_web_app.png)
...
1. On the **Resource groups** pane, select the **PubSubEvents** resource group.

1. On the **PubSubEvents** pane, select the **eventviewer**_[yourname]_ web app.

1. On the **App Service** pane, in the **Settings** category, select the **Properties** link.
...
1. On the **Resource groups** pane, select the **PubSubEvents** resource group that you created previously in this lab.

1. On the **PubSubEvents** pane, select the **hrtopic**_[yourname]_ Event Grid topic that you created previously in this lab.

1. On the **Event Grid Topic** pane, select **+ Event Subscription**.

1. On the **Create Event Subscription** pane, perform the following actions, and then select **Create**:
   ...
   ![Create Event Subscription blade](./Media/l09_create_event_subscription.png)
...
1. On the **Event Grid Topic** pane, record the value of the **Topic Endpoint** field. You'll use this value later in the lab.
...
1. In the **Main** method, perform the following actions to publish a list of events to your topic endpoint:
    ...
    a. Add the following line of code to create a new variable named **endpoint** of type **Uri** by using the **topicEndpoint** string constant as a constructor parameter:

        ```csharp
        Uri endpoint = new Uri(topicEndpoint); 
        ```

    b. Add the following line of code to create a new variable named **credential** of type **[AzureKeyCredential](https://docs.microsoft.com/dotnet/api/azure.azurekeycredential)** by using the **topicKey** string constant as a constructor parameter:

        ```csharp
        AzureKeyCredential credential = new AzureKeyCredential(topicKey);
        ```

    c. Add the following line of code to create a new variable named **client** of type **[EventGridPublisherClient](https://docs.microsoft.com/dotnet/api/azure.messaging.eventgrid.eventgridpublisherclient)** by using the **endpoint** and **credential** variables as constructor parameters:
    ...
    f. Add the following line of code to asynchronously invoke the **[EventGridPublisherClient.SendEventAsync](https://docs.microsoft.com/dotnet/api/azure.messaging.eventgrid.eventgridpublisherclient.sendeventasync)** method by using the **firstEvent** variable as a parameter:
    ...
    h. Add the following line of code to asynchronously invoke the **[EventGridPublisherClient.SendEventAsync](https://docs.microsoft.com/dotnet/api/azure.messaging.eventgrid.eventgridpublisherclient.sendeventasync)** method by using the **secondEvent** variable as a parameter:
